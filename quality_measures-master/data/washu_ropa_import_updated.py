...
def process_consult(data_lines, disease_site):
    ...
    for entry in performanceStatusEntries:
        ...
        if data_key == 'Performance Status':
            #performanceStatusSource['performanceStatusSource'] = value
            pass
    ...
    for entry in clinicalTrialsEntries:
        ...
        if data_key == 'Clinical Trials Date':
...
def process_follow_up(data_lines, disease_site):
    ...
    for line in followup_arrays['Follow-Up']:
        ...
        elif data_key == 'Patient Status':
            followUp[index]['patientStatus'] = value
...
def process_technical_validation(data_lines, disease_site):
    ...
    for line in technical_validation_arrays['External Beam Plan']:
        ...
        elif data_key == 'Frequency':   
            external_beam[index]['frequency'] = value
...
def reformat_data(json_data):
    ...
    if 'performanceStatus' in json_data:
        ...
        if len(json_data['performanceStatus']) > 0:
            current_consult['performanceStatus'] = json_data['performanceStatus']
    ...
    if history_label in json_data:
        ...
        #if 'smokingStatusSource' in json_data[history_label]:
            #current_consult['smokingStatusSource'] = json_data[history_label]['smokingStatusSource']
       ...
       # if 'implantableCardiacDeviceSource' in json_data[history_label]:
          # current_consult['implantableCardiacDeviceSource'] = json_data[history_label]['implantableCardiacDeviceSource']
    ...
    if 'followUp' in json_data:
        for follow_up in json_data['followUp']:
            ...
            if 'dateOfDeath' in follow_up:
                current_follow_up['dateOfDeath'] = follow_up['dateOfDeath']

            if 'patientStatus' in follow_up:
                current_follow_up['patientStatus'] = follow_up['patientStatus']
    ...
    if tumor_type_lower + 'Pathology' in json_data:
       ...
       # if 'pathologySource' in pathology:
          #  updated_data['pathology']['pathologySource'] = pathology['pathologySource']


        if 'isPathologyReport' in pathology:
            updated_data['pathology']['isPathologyReport'] = pathology['isPathologyReport']

        if 'pathologyDate' in pathology:
            updated_data['pathology']['pathologyDate'] = pathology['pathologyDate']
    ...
    if tumor_type_lower + 'Molecular' in json_data:
        ...
        #if 'molecularInformationSource' in molecular:
           # updated_data['molecular']['molecularInformationSource'] = molecular['molecularInformationSource']


        if 'molecularInformationReportDate' in molecular:
            updated_data['molecular']['molecularInformationReportDate'] = molecular['molecularInformationReportDate']
        ...
        if 'isMolecularInformationReport' in molecular:
            updated_data['molecular']['isMolecularInformationReport'] = molecular['isMolecularInformationReport']

        if 'molecularInformationReportType' in molecular:
            updated_data['molecular']['molecularInformationReportType'] = molecular['molecularInformationReportType']
    ...
    if tumor_type_lower + 'Chemotherapy' in json_data:
        ...
        for entry in chemotherapy:
            current_chemo_entry = {}
            if 'chemotherapyStartDate' in entry:
                current_chemo_entry['chemotherapyStartDate'] = entry['chemotherapyStartDate']
            if 'chemotherapyEndDate' in entry:
                current_chemo_entry['chemotherapyEndDate'] = entry['chemotherapyEndDate']

           # if 'chemotherapySource' in entry:
            #    current_chemo_entry['chemotherapySource'] = entry['chemotherapySource']

            if len(current_chemo_entry) > 0:
                updated_chemo.append(current_chemo_entry)
    ...
    if scoringAndGrades in json_data:
        ...
        #if 'overallTumorStageSource' in json_data[scoringAndGrades]:
          #  updated_diagnosis['clinicalStageSource'] = json_data[scoringAndGrades]['overallTumorStageSource']
        #else:
        #    updated_diagnosis['clinicalStageDate'] = "2010-01-01"
       ...
       # if 'nsclcClassificationSource' in json_data[scoringAndGrades]:
            #updated_diagnosis['nsclcClassificationSource'] = json_data[scoringAndGrades]['nsclcClassificationSource']
    ...
    if 'imagingStudies' in json_data['consult']:
        ...
        if 'ctOfBrainReportDate' in json_data['consult']['imagingStudies']:
            updated_data['diagnosticImaging']['ctDate'] = json_data['consult']['imagingStudies']['ctOfBrainReportDate']

        if 'ctOfBrainReportDate' in json_data['consult']['imagingStudies'] or 'ctOfBrainSource' in json_data['consult'][
            'imagingStudies'] or 'ctOfBrainMentionDate' in json_data['consult']['imagingStudies']:
            updated_data['diagnosticImaging']['ct'] = 'Yes'
        ...
        if tumor_type_lower.lower() + 'ExternalBeamPlan' in json_data:
            ...
            for tx in ebrt_data:
                ...
                if 'frequency' in tx:
                    current_ebrt_tx['frequency'] = tx['frequency']
                    #-- print("----------------------------------------------")
                    #-- print(current_ebrt_tx['frequency'])
                    #-- print("----------------------------------------------")
                    #exit(0)
        ...
        if 'isSimNoteOrSiteSetup' in other_data:
            other_information['isSimNote'] = other_data['isSimNoteOrSiteSetup']
        ...
        ## fixed dvhStructuresEvaluated for all disease_sites
        if 'nsclcDvhEvaluatedStructures' in other_data:
            other_information['dvhStructuresEvaluated'] = []
            for item in other_data['nsclcDvhEvaluatedStructures']:
                for x in item:
                    other_information['dvhStructuresEvaluated'].append(item[x])
                #-- print(item)
        if 'sclcDvhEvaluatedStructures' in other_data:
            other_information['dvhStructuresEvaluated'] = []
            for item in other_data['sclcDvhEvaluatedStructures']:
                for x in item:
                    other_information['dvhStructuresEvaluated'].append(item[x])
                #-- print(item)

        if 'planningAlgorithm' in other_data:
            other_information['planningAlgorithm'] = other_data['planningAlgorithm']

        if 'nsclcMotionManagement' in other_data:
            other_information['nsclcMotionManagement'] = []
            for item in other_data['nsclcMotionManagement']:
                for x in item:
                    other_information['nsclcMotionManagement'].append(item[x])

        if 'sclcMotionManagement' in other_data:
            other_information['sclcMotionManagement'] = []
            for item in other_data['sclcMotionManagement']:
                for x in item:
                    other_information['sclcMotionManagement'].append(item[x])
...
def main():
    ...
    try:
        with open('patient_ids.json') as json_data:
            d = json.load(json_data)
    except:
        with open('../data/patient_ids.json') as json_data:
            d = json.load(json_data)
    ...
    for line in data_file.readlines():
        if 'Mountain Home, TN' in line:
            line = line.replace('Mountain Home, TN', 'Mountain Home')
    ...
    print('Parsing Patients from WASHU Raw file:')
    for patient in tqdm(patient_map):
        json_data = {}
        data_lines = {}
        disease_site = None
        surgery = None
    ...
    with open('data.json', 'w') as outfile:
        json.dump(master_list, outfile, indent=4)
